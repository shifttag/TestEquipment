...
const int resetRegister[] = {E_0, E_1, E_2, E_3, E_4};

void reset()
{
  for (size_t i = 0; i < sizeof(resetPins) / sizeof((resetPins[0])); i++)
  {
    digitalWrite(resetPins[i], LOW);
  }
  for (size_t i = 0; i < sizeof(resetRegister) / sizeof(resetRegister[0]); i++)
  {
