...
count = 20       # 측정 갯수 설정
def get_serial_port_safe(port_name='COM8', baud_rate=115200, timeout=1, retries=5, delay=1):
    for attempt in range(retries):
        try:
            ...
            print(f"[WARNING] 포트 열기 실패 ({e}), {delay}초 후 재시도 {attempt+1}/{retries}")
            time.sleep(delay)
    raise Exception(f"[ERROR] 포트 {port_name}를 열 수 없습니다.")
    ...
    for cnt in range(1, count + 2):
        ...
        while time.time() < timeout:
            if ser.in_waiting:
                ...
                if line == f"conduction{cnt}_OK":
                    arduino_response = line
                    break
                else:
                    print(f"⚠ 예기치 않은 응답: {line}")
            time.sleep(0.05)
